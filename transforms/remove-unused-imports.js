export default function transformer(file, api, options) {
  ...
  const removeIfUnused = (importSpecifier, importDeclaration) => {
    ...
    const isUsedInPropertyDecorators = () => {
      let used = false
      root.find(j.ClassProperty).forEach(property => {
        used =
          used ||
          (property.node.decorators &&
            j(property.node.decorators)
              .find(j.Identifier, { name: varName })
              .size() > 0)
      })
      return used
    ...
    if (
      !(
        isUsedInScopes() ||
        isUsedInClassDecorators() ||
        isUsedInPropertyDecorators()
      )
    ) {
